...
public class P3Wrapper2 {
	...
	public static class BinarySearchTree<K, V> implements Map<K, V> {
		...
		/*
		 * Implement a member method isRightLinear which determines if a binary tree is
		 * right linear. A binary tree is right linear if every internal node has only a
		 * right child, and the left child is set to null. An empty tree and a tree with
		 * only one node are both right linear.
		 */
		public boolean isRightLinear() {
			...
			return isRightLinearAux(this.root, true);
		}

		public boolean isRightLinearAux(BinaryTreeNode<MapEntry<K, V>> N, boolean flag) {
			if (N == null) {
				return true;
			}
			if (N.getRightChild() != null) {
				return isRightLinearAux(N.getRightChild(), flag);
			}
			if (N.getLeftChild() != null) {
				return false;
			}
...
//	public boolean isRightLinearHelper(BinaryTreeNode<MapEntry<K,V>> N, boolean kek) {
//		if (N.getRightChild() != null) {
//			kek = isRightLinearHelper(N.getRightChild(), kek);
//		}
//		if (N.getLeftChild() != null) {
//			return false;
//		}
