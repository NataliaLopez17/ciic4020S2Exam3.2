...
public class P3Wrapper {
	...
	public static class BinarySearchTree<K, V> implements Map<K, V> {
		...
		/*
		 * Implement a member method lessThanValues which returns a Java Array List with
		 * all the key value pairs from the tree that have a key that is less than a
		 * given key, which is passed as parameter. Recall the private class MapEntry
		 * from the BinarySearchTree implements the interface KeyValuePair. The elements
		 * in the array list must be ordered in pre-order.
		 * 
		 */
		public ArrayList<KeyValuePair<K, V>> lessThanValues(K key) {
			ArrayList<KeyValuePair<K, V>> L = new ArrayList<KeyValuePair<K, V>>();
			if (this.root == null) {
				return L;
			}
			return lessThanValuesAux(key, L, this.root);
		}

		public ArrayList<KeyValuePair<K, V>> lessThanValuesAux(K key, ArrayList<KeyValuePair<K, V>> L,
				BinaryTreeNode<MapEntry<K, V>> N) {

			if (N == null) {
				return L;
			}
			if (this.keyComparator.compare(N.getValue().getKey(), key) < 0) {
				L.add(new MapEntry<K, V>(N.getValue().getKey(), N.getValue().getValue()));
			}
			lessThanValuesAux(key, L, N.getLeftChild());
			lessThanValuesAux(key, L, N.getRightChild());
			return L;
	...
	// public ArrayList<KeyValuePair<K, V>> lessThanValuesHelper(K key,
	// ArrayList<KeyValuePair<K, V>> L, BinaryTreeNode<MapEntry<K, V>> N) {
	// if (this.keyComparator.compare(N.getValue().key, key) < 0) {
	// L.add(N.getValue());
	// }
	// if (N.getLeftChild() != null) {
	// lessThanValuesHelper(key, L , N.getLeftChild());
	// }
	// if (N.getRightChild() != null) {
	// lessThanValuesHelper(key, L , N.getRightChild());
	// }
	// return L;
	//
	// }
