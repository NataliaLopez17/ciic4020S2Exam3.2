...
public class BucketPalsWrapper {
	...
	public static class HashtableSC<K, V> implements Map<K, V> {
		...
		public List<K> bucketPals(K key) {
			...
			List<MapEntry<K, V>> L = (List<MapEntry<K, V>>) this.buckets[targetBucket];
			List<K> result = new SinglyLinkedList<>();
			for (MapEntry<K, V> yeet : L) {
				if (!yeet.getKey().equals(key)) {
					result.add(yeet.getKey());
				}
			}
			return result;
		}
...
//		List<MapEntry<K,V>> L = (List<MapEntry<K, V>>) this.buckets[targetbucket];
//		if (L != null) {
//			List<K> lk = new SinglyLinkedList<>();
//			for (MapEntry<K, V> kek : L) {
//				if (kek.key != key) lk.add(kek.key);
//			}
//			return lk;
//		}
